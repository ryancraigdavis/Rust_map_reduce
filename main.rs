...
fn main() {
    ...
    // Clones xs to be used with the threads
    let xs2 = xs.clone();

    // Create 2 threads, move the xs data into them
    ...
    // Creates a vector of partitions based on the num of partitions passed, prints it
    ...
    // Loop that runs the map_data function for each of the partitions
    // by creating a thread for each partition and processing the data
    for t in 0..num_partitions {

        // Creates a clone for each vector processed by the threads
        let n_clone = ns.clone();

        // Creates a new thread and processes the vector at t - joins the thread
    ...
    // Prints the vector with the second group of intermediate sums
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // Partition size is based on the number of partitions passed
    let partition_size = v.len() / num_partitions;
    let mut xs: Vec<Vec<usize>> = Vec::new();

    // This value will allow the next loop to add 1 item to a certain number of
    // partitions
    let mut mod_value = v.len() % num_partitions;

    // The for loop starts with 0 and goes to partition size
    // But depending on the number of partitions, this is increased
    // with every cycle
    let mut start_num = 0;
    let mut end_num = partition_size;

    // Cycle through the num of partitions
    for i in 0..num_partitions {

        // Pushes each value of v into a partition
        ...
        // If mod > 0, it means the partition size is uneven
        // Add 1 value to the vector, and keep doing that while dec
        // Mod until mod is 0.
        if mod_value > 0 {
            xs[i].push(v[end_num]);
            ...
            // Resets the start and end nums by adding the partition size to them
            start_num = start_num + partition_size;
            end_num = end_num + partition_size;
        }
    }

    // Return xs - the final vector
    xs
}
